...
describe('POST /addProperty', () => {
  ...
  it('returns 500 when property creation fails', async () => {
    prismaMock.property.create.mockRejectedValue(new Error('DB error'));
    ...
    expect(res.body.error).toBe('Failed to create property');
  });

  it('returns 401 when user is not authenticated', async () => {
    const res = await request(app).post('/addProperty').send({});
